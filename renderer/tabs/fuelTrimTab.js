// Short Term Fuel Trim Analysis Tab Module
const FuelTrimTab = {
  elements: {
    avgTrim: null,
    maxDeviation: null,
  ...
  initialize() {
    ...
    this.elements.avgTrim = document.getElementById('fueltrim-avgTrim');
    this.elements.maxDeviation = document.getElementById('fueltrim-maxDeviation');
    this.elements.inTargetPercent = document.getElementById('fueltrim-inTargetPercent');
    this.elements.abnormalEvents = document.getElementById('fueltrim-abnormalEvents');
    this.elements.fuelTrimTableBody = document.getElementById('fueltrim-fuelTrimTableBody');
    this.elements.searchInput = document.getElementById('fueltrim-searchInput');
    this.elements.eventTypeFilter = document.getElementById('fueltrim-eventTypeFilter');
    ...
    document.querySelectorAll('#fueltrim-fuelTrimTable th[data-sort]').forEach(th => {
  ...
  render(analysisData) {
    if (!analysisData) {
      console.warn('Fuel trim tab: No analysis data available');
      this.showColumnInfo();
    ...
    if (analysisData.error) {
      console.warn('Fuel trim analysis error:', analysisData.error);
      this.showColumnInfo();
    }
  ...
  showColumnInfo() {
    ...
    const analyzer = tabManager ? tabManager.getTabAnalyzer('fueltrim') : null;
    ...
    if (!dataProcessor) {
      ...
      if (statsPanel) {
        let infoDiv = document.getElementById('fueltrim-column-info');
        if (!infoDiv) {
          infoDiv = document.createElement('div');
          infoDiv.id = 'fueltrim-column-info';
    ...
    if (statsPanel) {
      ...
      infoDiv.innerHTML = `
        <h3 style="margin-top: 0; color: #856404;">⚠️ Fuel Trim Column Not Found</h3>
        <p><strong>Looking for:</strong> "Fuel Trim - Short Term (%)" or similar</p>
  ...
  updateStatistics() {
    const analyzer = tabManager.getTabAnalyzer('fueltrim');
    if (!analyzer) {
      console.warn('Fuel trim analyzer not found');
      return;
    }

    const stats = analyzer.getStatistics();
    if (!stats) {
      console.warn('Fuel trim statistics not available');
      return;
    }
  ...
  renderCharts(preserveZoom = false) {
    const analyzer = tabManager.getTabAnalyzer('fueltrim');
    if (!analyzer) {
      console.warn('Fuel trim analyzer not found for charts');
      return;
    }
    ...
    const analysisData = tabManager.getCachedAnalysis('fueltrim');
    if (!analysisData) {
      console.warn('No cached fuel trim analysis data');
      return;
    }
    
    if (!data || data.length === 0) {
      console.warn('No data available for fuel trim charts');
      return;
    }
    ...
    const shortTermTrimsRaw = data.map(row => parseFloat(row[columns.shortTermTrim]) || 0);
    let shortTermTrims = breakAtGaps(shortTermTrimsRaw, times);
    ...
    if (window.applyDataSmoothing && window.smoothingConfig) {
      shortTermTrims = window.applyDataSmoothing(shortTermTrims, window.smoothingConfig.windowSize, window.smoothingConfig.enabled);
    ...
    const positivePoints = createEventPointArray(positiveEvents, e => e.shortTermTrim);
    const negativePoints = createEventPointArray(negativeEvents, e => e.shortTermTrim);
    ...
    const fuelTrimChartEl = document.getElementById('fueltrim-fuelTrimChart');
    if (fuelTrimChartEl) {
      const datasets = [
        {
        ...
        {
          label: 'Normal Range (+10%)',
          data: new Array(times.length).fill(10),
  ...
  updateTable() {
    ...
    if (searchTerm && searchTerm.trim() !== '') {
      ...
      filteredEvents = filteredEvents.filter(e => {
        return (
          ...
          e.shortTermTrim.toString().includes(term) ||
    ...
    const sortedEvents = [...filteredEvents].sort((a, b) => {
      ...
      switch (this.currentSort.column) {
        case 'time':
          ...
          break;
        case 'shortTermTrim':
          aVal = Math.abs(a.shortTermTrim);
          bVal = Math.abs(b.shortTermTrim);
          break;
        case 'rpm':
          aVal = a.rpm;
          bVal = b.rpm;
    ...
    sortedEvents.forEach(event => {
      ...
      const trimDisplay = event.maxShortTermTrim !== undefined ? event.maxShortTermTrim : event.shortTermTrim;
